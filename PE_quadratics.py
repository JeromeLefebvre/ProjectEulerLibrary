...
def isSquare(a):
    ...
    if 0 <= a <= 2 ** 50:
        ...
        if sr ** 2 == a:
            return sr
    # If a is large number, due to loss of precision, we use a purely integer
            ...
            for f in range(-10, 10):
                for a in range(-10, 10):
                    for b in range(-10, 10):
                        if a * b * e * f != 0:
                            solns = solveIntegerLinear(
                                e, a) + solveIntegerLinear(f, b)
                            solns.sort()
                            self.assertEqual(
                                solveIntegerQuadratic(e * f, f * a + e * b, a * b), solns)

        triangles = [0, 1, 3, 6, 10, 15, 21,
